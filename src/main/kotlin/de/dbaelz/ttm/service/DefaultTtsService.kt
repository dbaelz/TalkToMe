...
class DefaultTtsService(
    ...
    private val persistenceService: TtsJobPersistenceService,
    ...
    override fun generate(text: String, config: TtsConfig, engine: TtsEngine): TtsJob {
        val job = persistenceService.createJob(text = text, engine = engine, config = config)
    ...
    override fun getJob(id: String): TtsJob? = persistenceService.findJob(id)

    override fun getAllJobs(): List<TtsJob> = repo.findAll().mapNotNull { entity ->
        persistenceService.findJob(entity.id)
    }.toList()
